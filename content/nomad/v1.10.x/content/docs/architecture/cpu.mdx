...
No modo `none`, o agendador Nomad aproveita a apatia de jobs sem preferência de
afinidade NUMA para ajudar a reduzir a fragmentação do núcleo dentro dos nós
NUMA.
Ele faz isso agrupando a requisição de núcleo desses jobs nos nós NUMA com o
...
`devices` é uma lista opcional de dispositivos que devem ser colocados no nó
NUMA junto com núcleos de CPU alocados.
