...
Remote Task Drivers allow custom task driver plugins to execute tasks using nonlocal
runtimes such as cloud container runtimes. Without this support, task driver plugins
trying to manage remote tasks would run into the following problems:
...
The Remote Task Driver feature improves these behaviors for custom plugins that
 advertise the [`RemoteTasks` capability][remote-cap].
