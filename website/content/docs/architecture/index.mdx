...
clientes e
[calcular a alocação das tarefas](/nomad/docs/concepts/scheduling/how-scheduling-works).
...
O Nomad é otimistamente concorrente, o que significa que todos os servidores
...
servidores.
Um job representa um estado desejado e fornece o conjunto de tarefas que devem
ser executadas.
Os servidores são responsáveis pelo agendamento das tarefas, o que é feito
encontrando uma alocação ideal para cada tarefa, de forma que a utilização de
recursos seja maximizada, satisfazendo todas as restrições especificadas pelo
job.
A utilização de recursos é maximizada pelo bin packing, no qual o agendamento
